...
do
    TS_FILE="ts_$FILE"
    cat $FILE | grep -E "^[0-9]+:[0-9]+:[0-9]+ .+" | awk '
        BEGIN{ts=0; last_ts=0;}
        {
			split($1, t, ":")
...
cat $TS_ALL_SORTED | awk -v r=$USERS_REGEXP '{if ($6 ~ r) print}' > $TS_ALL_SORTED_USERS

cat $TS_ALL_SORTED_USERS | awk  -v lic=$LICENSE '{if ($5 == lic && ($4 == "IN:" || $4 == "OUT:")) print $1 " " $4 " " $6 " " $5}' > $TS_ALL_SORTED_LICENSE
...
cat $TS_ALL_SORTED_LICENSE | awk -v mts=$MIN_TS 'BEGIN{out=0; last_check_ts=0}
{
	...
	if ($1 > last_check_ts) {
		for (node in last) {
			if (lic[node]> 0 && ($1 > last[node] + 2592000)) { # 30 days
				lic[node]=0
				out--
				delete last[node]
			}
...
}
{
	...
	if (day != $1) {
			...
			while ((curr_d[1] != last_d[1]) || (curr_d[2] != last_d[2]) || (curr_d[3] != last_d[3])) {
				print last_d[1]"/"last_d[2]"/"last_d[3]" 0"
